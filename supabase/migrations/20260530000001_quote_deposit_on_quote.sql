-- ════════════════════════════════════════════════════════════════════════════
-- CaterOS · Honor the per-quote deposit + expose it on the public quote
-- ════════════════════════════════════════════════════════════════════════════
-- Two fixes:
--   1. accept_quote() now uses the quote's own deposit_cents when set (> 0),
--      falling back to the org-level deposit_rate otherwise. Previously the
--      per-quote deposit was silently ignored.
--   2. get_quote_by_token() returns deposit_due_cents (the effective deposit)
--      so the public quote view can show "Deposit due to book" and offer a
--      pay-with-card button before/at acceptance.

-- ─── accept_quote: prefer the quote's own deposit_cents ─────────────────────
...
begin
  ...
  -- Per-quote deposit wins when set; otherwise fall back to the org rate.
